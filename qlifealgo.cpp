   ...
   for (int i=0; i<2; i++) {
     ...
     if (rootlev > 1)
       root->flags = 0xf0000000 |
         (upchanging(oroot->flags) << (3 + (generation & 1))) ;
     root->d[4] = oroot ;
     if (oroot != nullroot) {
...
#define USEPOPCOUNT
#ifdef USEPOPCOUNT
                  pop += __builtin_popcount(pp->b[i]->d[j+gm1*8]) ;
#else
                  k = pp->b[i]->d[j+gm1*8] ;
                  if (k)
...
G_INT64 qlifealgo::popcount() {
   return find_set_bits(root, rootlev, generation & 1) ;
}
int qlifealgo::isEmpty() {
   return isEmpty(root, rootlev, generation & 1) ;
}
/*
...
public:
   ...
   virtual lifealgo *createInstance() {
      return new qlifealgo() ;
   }
} factory ;
qlifealgofactory::qlifealgofactory() {
   registerAlgo("qlife", &factory) ;
}
