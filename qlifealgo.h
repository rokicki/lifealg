...
public:
   ...
   virtual int getpopulation() { return (int)popcount() ; }
   ...
   void setIncrement(int inc) { increment = inc ; }
   int isEmpty() ;
   // can we do the gen count doubling? only hashlife
   int hyperCapable() { return 0 ; }
   void setMaxMemory(int m) ;
   int getMaxMemory() { return (int)(maxmemory >> 20) ; }
   const char *setrule(const char *s) ;
   const char *getrule() { return qliferules.getrule() ; }
   void step() ;
   void* getcurrentstate() { return 0 ; }
