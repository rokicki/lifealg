...
class bitpar2algo : public lifealgo {
public:
...
static class bitpar2algofactory : public lifealgofactory {
public:
   bitpar2algofactory() ;
   virtual lifealgo *createInstance() {
      return new bitpar2algo() ;
   }
} factory ;
bitpar2algofactory::bitpar2algofactory() {
   registerAlgo("bitpar2", &factory) ;
}
void bitpar2algo::init(int w_, int h_) {
   w = w_ ;
   h = h_ ;
   wordwidth = (w + 63) >> 6 ;
   wh = wordwidth * h ;
...
void bitpar2algo::setcell(int x, int y) {
   u0[(x>>6)*h+y] |= 1LL << (x & 63) ;
}
int bitpar2algo::getpopulation() {
   int r = 0 ;
   for (int i=0; i<wh; i++)
...
void bitpar2algo::swap() { ::swap(u0, u1) ; }
int bitpar2algo::nextstep(int i, int n, int needpop) {
   int r = 0 ;
